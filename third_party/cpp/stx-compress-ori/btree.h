...
private:
    ...
    //huanchen-compress
    struct compressed_node : public node
    {
      typedef typename _Alloc::template rebind<compressed_node>::other alloc_type;

      compressed_node *prevleaf;
      compressed_node *nextleaf;
...
public:
	...
	inline void moveToPrev() {
	  ...
	  return tmp;
        ...
        /// Equality of iterators
        inline bool operator == (const static_iterator& x) const
        {
            return (x.currnode_compressed == currnode_compressed) && (x.currslot == currslot);
        }
    ...
    public:
        ...
        inline hybrid_iterator()
          : currnode(NULL), currnode_static_compressed(NULL), currslot(0), currslot_static(0), currtree(0), kless(NULL)
        { }

        inline hybrid_iterator(key_compare kl)
          : currnode(NULL), currnode_static_compressed(NULL), currslot(0), currslot_static(0), currtree(0), kless(kl)
        { }

        /// Initializing-Constructor of a mutable iterator
        inline hybrid_iterator(typename btree::leaf_node* l, short s, typename btree::compressed_node* l_static_compressed, short s_static, unsigned short ctree, key_compare kl)
	  ...
	  if (l_static_compressed != NULL) {
        ...
        inline hybrid_iterator(typename btree::leaf_node* l, short s, typename btree::compressed_node* l_static_compressed, const typename btree::leaf_node* l_static, short s_static, unsigned short ctree, key_compare kl)
          : currnode(l), currslot(s), currnode_static_compressed(l_static_compressed), currslot_static(s_static), currtree(ctree), kless(kl)
        { 
	  memcpy(&currnode_static, l_static, sizeof(leaf_node));
        ...
        inline compressed_node *get_currnode_static_compressed() {
	...
	inline void moveToPrev() {
	  ...
	  if (currnode_static_compressed == NULL) {
	    (x.currnode == currnode) && (x.currslot == currslot) && (x.currtree == currtree);
          ...
          return (x.currnode == currnode) && (x.currslot == currslot) && (x.currnode_static_compressed == currnode_static_compressed) && (x.currslot_static == currslot_static) && (x.currtree == currtree);
        }
...
private:
    ...
    compressed_node* m_headleaf_static; //huanchen-compress
    ...
    uint32_t inner_size;
    uint32_t inner_static_size;
    uint32_t compressed_size;

    uint32_t leaf_dealloc_size;
    uint32_t leaf_static_dealloc_size;
    uint32_t inner_dealloc_size;
    uint32_t inner_static_dealloc_size;
    uint32_t compressed_dealloc_size;
...
private:
    ...
    typename compressed_node::alloc_type compressed_node_allocator()
    {
        return typename compressed_node::alloc_type(m_allocator);
    }
...
*/
    ...
    inline void free_node(node* n)
    {
      //std::cout << "free node\n";
        if (n->isleafnode()) {
            ...
            a.destroy(ln);
            a.deallocate(ln, 1);
	    leaf_dealloc_size += sizeof(leaf_node);
            m_stats.leaves--;
        }
        else {
            ...
            a.destroy(in);
            a.deallocate(in, 1);
	    inner_dealloc_size += sizeof(inner_node);
            m_stats.innernodes--;
        }
    }
    ...
    inline void free_node_static(node* n)
    {
        if (n->isleafnode()) {
            ...
            a.destroy(ln);
            a.deallocate(ln, 1);
	    leaf_static_dealloc_size += sizeof(leaf_node);
            m_stats_static.leaves--;
        }
        else {
            inner_node* in = static_cast<inner_node*>(n);
            ...
            a.destroy(in);
            a.deallocate(in, 1);
	    inner_static_dealloc_size += sizeof(inner_node);
            m_stats_static.innernodes--;
        }
    }
    ...
    inline void free_node_compressed(compressed_node* n)
    {
      ...
      a.destroy(n);
      a.deallocate(n, 1);
      compressed_dealloc_size += sizeof(compressed_node);
      m_stats.leaves--;
    }
...
public:
    ...
    void clear_static()
    {
	    ...
	    if (m_root_static->isleafnode()) {
	      compressed_node *cn = reinterpret_cast<compressed_node*>(m_root_static);
...
public:
	...
	const leaf_node* leaf = reinterpret_cast<const leaf_node*>(str.data());
      ...
      ? hybrid_iterator(NULL, 0, leaf_compressed, leaf, slot, 1, m_key_less) : hybrid_end();
    }
    ...
    hybrid_iterator find_hybrid(const key_type& key)
    {
      ...
      return hybrid_iterator(key_iter.get_currnode(), key_iter.get_currslot(), NULL, 0, 0, m_key_less);
    }
	...
	const leaf_node* leaf = reinterpret_cast<const leaf_node*>(str.data());
        ...
        while (leaf_compressed && slot < leaf->slotuse && key_equal(key, leaf->slotkey[slot]))
        {
            ++num;
            if (++slot >= leaf->slotuse)
            {
    ...
    //huanchen-comrpess
    static_iterator lower_bound_static(const key_type& key)
    {
        node* n = m_root_static;
    ...
    hybrid_iterator lower_bound_hybrid(const key_type& key)
    {
      ...
      if ((iter == end()) && iter_static.isEnd()) {
        ...
        return hybrid_iterator(m_tailleaf, m_tailleaf ? m_tailleaf->slotuse : 0, iter_static.get_currnode_compressed(), iter_static.get_currnode(), iter_static.get_currslot(), 1, m_key_less);
      }
        ...
        return hybrid_iterator(iter.get_currnode(), iter.get_currslot(), iter_static.get_currnode_compressed(), iter_static.get_currnode(), iter_static.get_currslot(), 0, m_key_less);
      }
      else {
        return hybrid_iterator(iter.get_currnode(), iter.get_currslot(), iter_static.get_currnode_compressed(), iter_static.get_currnode(), iter_static.get_currslot(), 1, m_key_less);
      }
    }
...
public:
	...
	bloom_filter_static = CreateEmptyFilter_static(m_stats_static.itemcount + m_stats.itemcount);
      ...
      int node_count = 0; //node count per level
      int inner_level = 1;
	...
	leaf_node *new_ln = allocate_leaf_static();
        while (ln != NULL) {
	      ...
	      compressed_node *new_ln_static = allocate_leaf_compressed();
            ...
            new_ln->slotkey[curslot] = ln->slotkey[slot];
	...
	const leaf_node *ln_static_leaf;
        int slot = 0;
        int slot_static = 0;
        int slot_new = 0;
	bool ln_static_change = true;
